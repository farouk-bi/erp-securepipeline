...
class SecurityGate:
    ...
    def parse_trivy_sca(self):
        ...
        if not os.path.exists(filepath):
            print(f"⚠️  Trivy SCA report not found: {filepath}")
            return
    ...
    def parse_trivy_image(self):
        ...
        if not os.path.exists(filepath):
            print(f"⚠️  Trivy Image report not found: {filepath}")
            return
    ...
    def parse_gitleaks(self):
        ...
        if not os.path.exists(filepath):
            print(f"⚠️  GitLeaks report not found: {filepath}")
            return
    ...
    def parse_zap(self):
        ...
        if not os.path.exists(filepath):
            print(f"⚠️  ZAP report not found: {filepath}")
            return
    ...
    def evaluate_gate(self, gate_number=1):
        ...
        if self.results["total_high"] > 0:
            passed = False
            reasons.append(f"❌ {self.results['total_high']} vulnérabilités HIGH détectées")
        ...
        if self.results["total_medium"] > 5:
            reasons.append(f"⚠️  {self.results['total_medium']} vulnérabilités MEDIUM (> seuil de 5)")
            # Optionnel : décommenter pour rendre bloquant
            # passed = False
